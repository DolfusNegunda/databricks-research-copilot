...
Own copy for pipelines/ -- see pipelines/lakebase.py's docstring for why
embed_papers.py can't reach the root copy via sys.path.insert(Path(__file__))
the way app/ and mcp_server/ do (spark_python_task execs this file without
__file__ in its globals). This one has no __file__ dependency of its own, so
it's a verbatim copy, not an adapted one.
