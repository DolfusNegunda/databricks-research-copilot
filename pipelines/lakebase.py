...
Own copy for pipelines/, not an import of the root lakebase.py: this file is
imported by sync_to_lakebase.py, which Databricks runs as a spark_python_task
via exec(compile(source, filename, 'exec')) inside an IPython kernel rather
than a plain `python script.py` subprocess -- __file__ is never injected into
that exec'd namespace, so the root copy's sys.path.insert(Path(__file__)...)
trick NameErrors before it ever reaches this import. A module reached via a
normal `import lakebase`, unlike the exec'd top-level script, still gets a
correctly-set __file__ from Python's own import machinery -- so this copy's
_SQL_DIR below works fine; the fix is avoiding sys.path entirely by living in
the same workspace directory as the script that imports it, not patching
__file__ resolution.
...
This project shares one Lakebase instance with three sibling projects on the
same secret (`database`/`lakebase-url`) -- see the workspace root CLAUDE.md.
LAKEBASE_SCHEMA defaults to "research" specifically so this can never collide
with `support`/`public`/`weather`.
"""
...
# same Lakebase instance. Interpolated into SQL text below (you cannot bind
# an identifier), so it is validated as a bare identifier rather than trusted
# as-is.
...
def _connect_kwargs() -> dict:
    ...
    return {
        ...
        "application_name": "databricks-research-copilot-pipeline",
    }
...
def get_connection():
    ...
    every time -- so every caller can write plain `papers`/`citation_edges`
    and land in the right schema without qualifying a single table name. This
    deliberately uses a SET statement rather than a libpq `options=-c
    search_path=...` connect parameter: Lakebase sits behind a connection
...
def get_engine():
    ...
    only needs get_connection()/run_query()/run_write() -- e.g. embed_papers.py,
    whose serverless environment never declared SQLAlchemy as a dependency
    (sync_to_lakebase.py's does; embed's doesn't, confirmed live) -- doesn't
...
def ensure_research_schema(embedding_dim: int = 384) -> dict:
    """Apply sql/01_schema.sql. Fully IF NOT EXISTS / idempotent, safe to call
    on every app boot and every job/notebook run.

    Failures are captured, never raised: this runs during app startup or job
    setup, and an exception escaping there kills the deployment before it can
    report why anywhere useful.
    """
