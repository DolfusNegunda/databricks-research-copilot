...
def _connect_kwargs() -> dict:
    ...
    return {
        ...
        "application_name": "databricks-research-copilot",
    }
...
def _resolve_vector_extension_schema(conn) -> str | None:  # noqa: ANN001
    ...
    schema-bootstrap run widens search_path for the duration of its own
    connection only -- SET is session-scoped, and this same get_connection()
    resets search_path to the plain two-schema form on every checkout, by
    design, so that widening never survives past the one call that did it.
    Every *other* caller needs the same widening applied on its own
    checkouts too, which is what this function feeds.
    """
...
def get_connection():
    ...
    not already LAKEBASE_SCHEMA/public, so every `::vector` cast and every
    `<=>` distance query resolves the type regardless of which project on
    this shared instance happened to install the extension first.
