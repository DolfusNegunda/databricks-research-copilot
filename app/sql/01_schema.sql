...
-- Own copy for app/ -- see app/lakebase.py's _SQL_DIR: Databricks Apps
-- deployment only uploads files from within this app's own folder, never
-- sibling directories like the repo-root sql/ this was copied from
-- (confirmed live: FileNotFoundError on '/app/python/sql/01_schema.sql'
-- on every deploy, silently caught by ensure_research_schema()'s own
-- error handling, so nothing crashed -- it just never actually bootstrapped
-- the schema from this app, relying entirely on the Job-based path having
-- already done it). Keep in sync by hand with the root sql/01_schema.sql.
--
